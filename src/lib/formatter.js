var _ = require('underscore'),
    SHORT = _.template('Languages: jw.org: <%= jworg.total %>, <%= jworg.hasWebContent %> L1+, <%= jworg.isSign %> SLs, <%= jworg.isRTL %> RTL, <%= jworg.isSignWithWeb %> L1+ SLs; jwb: <%= jwb.web.total %>, <%= jwb.roku.total %> Roku, <%= jwb.web.isSign %> SLs, <%= jwb.web.isLangPair %> pairs'),
